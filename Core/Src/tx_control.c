...
uint16_t    numBloqFirm1 = 0;
uint16_t    cntBloqFirm1 = 0;
...
//JTA
void tx_control_write_firm (void)
{
	Bloque_handshake[comando1] = 0xF1; 				// comando1 = 0xF1;
	Bloque_handshake[comando2] = 0x03;   		 	// comando2 = 0x03;
	flagsRxFirm[0] = 1;
...
void rx_firmware(void)
{
	...
	uint32_t cksum_gral_aux1 = 0;
	uint32_t cksum_gral_aux2 = 0;
	uint32_t cksum_gral_aux3 = 0;
	uint32_t cksum_gral_aux4 = 0;

	//uint32_t cksum_aux = 0;

	uint32_t cksum_aux1 = 0;
	uint32_t cksum_aux2 = 0;
	uint32_t cksum_aux3 = 0;
	uint32_t cksum_aux4 = 0;

	uint16_t numbloqaux1 = 0;
	uint16_t numbloqaux2 = 0;

	//bset	flagsLogger,#4;				/ cancela loggeo de eventos
	//bset	flagsLogger,#5;
	flagsLogger[4] = 1;
	flagsLogger[5] = 1;
	...
	if(flagsRxFirm[3] == 0)
	{
		...
		if(flagsRxFirm[1] == 1)
		{
			...
			if(flagsRxFirm[2] == 1)
			{
				...
				for(uint8_t i = 0; i < 128; i++)
				{
					chksum_32_HW_LW = chksum_32_HW_LW + RxBuffer_Ble[i];
				}
				cksum_aux1 = RxBuffer_Ble[128] << 24;
				cksum_aux2 = RxBuffer_Ble[129] << 16;
				cksum_aux3 = RxBuffer_Ble[130] << 8;
				cksum_aux4 = RxBuffer_Ble[131];
				cksum_aux = cksum_aux1 | cksum_aux2 | cksum_aux3 | cksum_aux4;

				if(cksum_aux == chksum_32_HW_LW)
				{
					chksumFirm = chksumFirm + chksum_32_HW_LW;

					borra_pagina_logger();
					graba_bloque_fw();

					cntBloqFirm1++;
					if(cntBloqFirm1 == numBloqFirm1)
					{

						flagsRxFirm[3] = 1;
						direccion_fw = 0x8018800;
						direccion_borrado = 0x8018800;
						//rxFirm_noComplete:
						grabacion_exitosa_handshake();
//						Bloque_handshake[comando1] = 0xF1; 				// comando1 = 0xF1;
//						Bloque_handshake[comando2] = 0x3D;
						timeOutRx  = 5;
						//ok_writeBloqFirm:
						manda_transmision ();
					}
...
//						Bloque_handshake[comando2] = 0x3D;
				...
				{
					//no_writeBloqFirm:
					recepcion_inc_bloq_fw();
//					Bloque_handshake[comando1] = 0xF1; 				// comando1 = 0xF1;
//					Bloque_handshake[comando2] = 0x3E;
//
//					for(uint8_t i = 0; i < 8; i++)
//					{
//						flagsRxFirm[i] = 0;
...
//					bandera_act_fw_j = 0;          //Parche
		...
		else
		{
			...
			{
				//rx_numBloqFirm_01:
				numbloqaux1 = RxBuffer_Ble[2] << 8;
				numbloqaux2 = RxBuffer_Ble[3];

				numBloqFirm1 = numbloqaux1 | numbloqaux2;

				//numBloqFirm1 = RxBuffer_Ble[2];
				cntBloqFirm1 = 0;

				Bloque_handshake[comando1] = 0xF1; 				// comando1 = 0xF1;
				Bloque_handshake[comando2] = 0x07;   		 	// comando2 = 0x03;
				...
				manda_transmision ();
				timeOutRx = 5;
				//end_rx_firmware : end_tx_control
	...
	else
	{
		...
		if(!flagsRxFirm[4])
		{
			if(codeTX == 0x4A)
			{
				cksum_gral_aux1 = RxBuffer_Ble[2] << 24;
				cksum_gral_aux2 = RxBuffer_Ble[3] << 16;
				cksum_gral_aux3 = RxBuffer_Ble[4] << 8;
				cksum_gral_aux4 = RxBuffer_Ble[5];

				cksum_gral_aux = cksum_gral_aux1 | cksum_gral_aux2 | cksum_gral_aux3 |cksum_gral_aux4;

				if(cksum_gral_aux == chksumFirm)
				{
					grabacion_exitosa_handshake();
//					Bloque_handshake[comando1] = 0xF1; 				// comando1 = 0xF1;
//					Bloque_handshake[comando2] = 0x3D;
...
void graba_bloque_fw (void)
{
	...
	uint32_t contenido1 = 0;
	uint32_t contenido2 = 0;
	uint32_t contenido3 = 0;
	uint32_t contenido4 = 0;
	...
	for(i = 0; i < 32; i++)
	{
		switch (j)
		{
			...
			case 3:
				...
				buffer_recepcion = contenido1 | contenido2 | contenido3 | contenido4;
...
void borra_pagina_logger (void)
{
	FLASH_EraseInitTypeDef erase;
	erase.TypeErase = FLASH_TYPEERASE_PAGES;
	erase.NbPages = 1;
	//erase.PageAddress = direccion_borrado;
	uint32_t err = 0;

	//HAL_FLASH_Unlock();
	//while(HAL_FLASH_Unlock() !=  HAL_OK );
	//while(HAL_FLASHEx_Erase (&erase, &err) !=  HAL_OK );
	//HAL_FLASHEx_Erase (&erase, &err);
	//HAL_FLASH_Lock();

	direccion_borrado = direccion_borrado + 0x80;
}

/*void borra_logger (void)
{
	FLASH_EraseInitTypeDef erase;
	erase.TypeErase = FLASH_TYPEERASE_PAGES;
	erase.NbPages = 480;
	erase.PageAddress = 0x8018800;
	uint32_t err = 0;
